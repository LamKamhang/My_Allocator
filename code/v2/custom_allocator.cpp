...
void timer_test(const int &sz) {
    ...
    // elapsed time array
    vector<long long> elapse_self_new, elapse_stl_default, elapse_mem_pool, elapse_myalloc;
    struct teststruct {
        char buf[1024];
    };
...

    {
      ...
      // ---------------------------------------
      {
        ...
        // create fix sized container
        start_time = steady_clock::now();
        vector<int, MyAllocator<int>> vfix(sz);
        end_time = steady_clock::now();
        elapse_myalloc.push_back(
          duration_cast<microseconds>(end_time - start_time).count());
        vfix.clear();

        // container manipulation
        // cout << "vector grow test" << endl;
        vector<teststruct, MyAllocator<teststruct>> v;
        start_time = steady_clock::now();
        for (auto i = 0; i < sz; ++i) {
          if (mydist(myeng) % 2 || v.empty())
            v.push_back(teststruct());
      ...
      }
      {
        ...
        // create fix sized container
        start_time = steady_clock::now();
        list<int, MyAllocator<int>> lfix(sz);
        end_time = steady_clock::now();
        elapse_myalloc.push_back(
          duration_cast<microseconds>(end_time - start_time).count());
        lfix.clear();
        ...
        // container grow
        // cout << "list grow test" << endl;
        list<teststruct, MyAllocator<teststruct>> l;
        start_time = steady_clock::now();
        for (auto i = 0; i < sz; ++i) {
          if (mydist(myeng) % 2 || l.empty())
            l.push_back(teststruct());
      ...
      }
      {
        ...
        // container grow
        // cout << "map grow test" << endl;
        map<int, int, std::less<int>, MyAllocator<pair<int, int>>> m;
        // map<int, int> m;
        start_time = steady_clock::now();
        for (auto i = 0; i < sz; ++i)
          if (mydist(myeng) % 2 || m.empty())
      ...
      std::copy(elapse_myalloc.begin(), elapse_myalloc.end(),
        std::ostream_iterator<long long>(cout, ","));
      cout << endl;
    }
}